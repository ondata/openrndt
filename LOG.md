...
- **`references/ogc-services.md`**: CRS del WMS catastale e retry sul WFS (`&_=1`), con uno snippet provato dal vivo.
...
- **`docs/segnalazioni-rndt-agid.md`** → docx via Quarto con `references/reference.docx` (cartella e `docs/*.docx` in `.gitignore`). Sette punti con comando e stato, quattro osservazioni nuove (bbox malformata, `updated`, OR implicito, item 200/`found:false`), tre note sulla pagina ufficiale. Riverifica prevista nella prima settimana di ottobre.
- **Riverifica completa del 2026-09-04, catalogo a 23.741**. Risolti oltre al 4: il 5 per via documentale e la parte documentale del 2, perché la pagina «Accesso ai dati tramite API REST» è stata aggiornata il 31/07/2026 (`sort` con i soli valori reali, `dataCategory` sparito, nuovo parametro `modified=da,a`). Il 3 a metà: l'esempio §2.2.1 della guida CSW dà 824 record (era 0), il GetCapabilities dichiara i queryables apiso dell'Allegato A e funzionano (nomi case-sensitive: `apiso:title` sì, `apiso:Title` no), il conteggio a vuoto è `"0"`. Restano `SortBy` ignorato, nessun operatore di intervallo (`PropertyIsBetween` «not supported») e una regressione: `constraint` in KVP ignorato in silenzio, qualunque vincolo → catalogo intero (a luglio `dct:modified` 2025 dava 0). Aperti 1, 6, 7 con numeri invariati.
- **Difetti nella pagina aggiornata**: `keyword_s` e `isOpenData` scritti male (reali `keywords_s`, `isOpendata`; case-sensitive, con i nomi in pagina → 0); `modified` filtra su `sys_modified_dt` (indicizzazione: `2025` → 0 contro 10.424 su `apiso_Modified_dt`, `2026-04-25,26` → 19.050) e non sulla data del metadato; `num` max dichiarato 5000 ma 6000 → 200 con 6.000 risultati.
- **Chiarita la contraddizione su `sort=title`** in `known-issues.md`: il nudo ha sempre dato 400 (Fielddata), a funzionare è `title:asc|desc` (ordine per byte). Tabella delle segnalazioni aggiornata al 4 settembre.

## 2026-09-03 (corretto: `get` con UUID senza prefisso)

- **Rilevato e corretto il falso negativo di `get` su UUID nudo**: `get 7832b30d-8e4a-4900-836d-1d4e960c3325` (scheda PAI dissesti Sicilia) rispondeva «Metadato non trovato» perché l'endpoint item risolve solo la forma `prefisso:uuid`. Nuovo `resolve_item_id` (`item.py`): risolve l'UUID via ricerca tenendo l'unico risultato che lo contiene nell'ID; zero risultati → `ItemNotFoundError`, più di uno → nuova `AmbiguousItemIdError` con i candidati. Collegato a `get` (anche `--xml`/`--html`), `resources` e `search --id`; esportato da `openrndt`. Otto test nuovi (166 totali), ruff e mypy `strict` puliti, smoke test live su scheda PAI. Changelog in `[Unreleased]`, skill Fase 3 aggiornata.
