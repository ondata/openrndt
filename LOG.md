...
- **v1.0.0** (ottavo punto, parziale su richiesta esplicita): `CHANGELOG.md` (Keep a Changelog), bump versione in `pyproject.toml`/`__init__.py`, classifier PyPI `3 - Alpha` → `5 - Production/Stable`, README aggiornato. Build 1.0.0 verificata (`uv build` + `twine check` PASSED, `py.typed` nel wheel). **Nessun push, tag o pubblicazione PyPI**: su richiesta dell'utente, questo giro si ferma al commit locale — tag/push/publish restano un passo successivo esplicito.
