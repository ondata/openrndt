...
valore. Le sette della busta (`_index`, `_id`, `_version`, `_seq_no`,
`_primary_term`, `found`, `_source`) restano identiche e i campi nuovi si
aggiungono in cima, quindi i `jq` scritti per le versioni precedenti continuano
...
- **`--raw` con `--xml`/`--html` è un errore esplicito** invece di essere ignorato in silenzio, come già `--xml` insieme a `--html`.
- **Helper del documento normalizzato esposti come API di libreria**: `item_record`, `contact_point`, `download_urls`, `bbox_from_envelope`.
...
- Il set di regole di ruff è dichiarato esplicitamente in `pyproject.toml` (`E`, `W`, `F`, `I`): il default (`E4`, `E7`, `E9`, `F`) non vedeva spazi in coda e import fuori ordine.
